...
| E04 | 可信执行、隔离与根资源预算 | in_progress（根预算/broker已verified） | 主控预算14、broker9、executor6项测试及fmt/clippy通过；真实提供商、进程隔离尚未验。集成发现group>10000停止扫描缺陷，修复复验中。 |
| E05 | 独立验收器与有边界的统计判定 | implemented_not_verified | 主控流式9项与旧评测6项初验通过；晚到审计修复待复验，大group停止扫描待修复；fixture报告强制inconclusive/禁止生产晋级，完整成本与分进程评分仍待验。 |
| E06 | 组合发布、实际应用与最小撤销闭环 | implemented_not_verified | 子代理已实现组合发布存储、独立批准/CAS、实际应用与实时撤销门；主控独立验收待完成，不采用子代理自报通过。 |
